...
using Crimo.Items.Guns;
using Crimo.Dusts;
using Crimo.Projectiles;

namespace Crimo.NPCs
{
	[AutoloadHead]
	class Micro : ModNPC
	{
		...
		public override bool PreAI() {
			if ((!Main.dayTime || Main.time >= despawnTime) && !IsNpcOnscreen(NPC.Center)) // If it's past the despawn time and the NPC isn't onscreen
			{
				...
				if (Main.netMode == NetmodeID.SinglePlayer) Main.NewText(Language.GetTextValue("Micro", NPC.FullName), 50, 125, 255);
				else ChatHelper.BroadcastChatMessage(NetworkText.FromKey("Micro", NPC.GetFullNetName()), new Color(50, 125, 255));
				NPC.active = false;
				NPC.netSkip = -1;
				NPC.life = 0;
				return false;
		...
		public static void UpdateTravelingMerchant() {
			bool travelerIsThere = (NPC.FindFirstNPC(ModContent.NPCType<QXWK>()) != -1); // Find a Merchant if there's one spawned in the world
			...
			if (!travelerIsThere && CanSpawnNow()) {
				int newTraveler = NPC.NewNPC(Terraria.Entity.GetSource_TownSpawn(), Main.spawnTileX * 16, Main.spawnTileY * 16, ModContent.NPCType<QXWK>(), 1); // Spawning at the world spawn
		...
		public void CreateNewShop() {
			...
			switch (Main.rand.Next(2)) {
				case 0:
					itemIds.Add(ModContent.ItemType<WateringCan>());
					break;
				default:
					itemIds.Add(ModContent.ItemType<WateringCan>());
					break;
		...
		public override string GetChat() {
			...
			if (partyGirl >= 0) {
				chat.Add(Language.GetTextValue("喵呜~", Main.npc[partyGirl].GivenName));
			}

			chat.Add(Language.GetTextValue("喵？"));

			string MicroDialogue = Language.GetTextValue("喵喵喵");
			chat.Add(MicroDialogue);

			string dialogueLine = chat; // chat is implicitly cast to a string.
			if (MicroDialogue.Equals(dialogueLine)) {
			}
