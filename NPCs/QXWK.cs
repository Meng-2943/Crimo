	...
	class QXWK : ModNPC
	{
		...
		public override bool PreAI() {
			if ((!Main.dayTime || Main.time >= despawnTime) && !IsNpcOnscreen(NPC.Center)) // If it's past the despawn time and the NPC isn't onscreen
			{
				...
				if (Main.netMode == NetmodeID.SinglePlayer) Main.NewText(Language.GetTextValue("青翔未阔", NPC.FullName), 50, 125, 255);
				else ChatHelper.BroadcastChatMessage(NetworkText.FromKey("青翔未阔", NPC.GetFullNetName()), new Color(50, 125, 255));
				NPC.active = false;
				NPC.netSkip = -1;
				NPC.life = 0;
				return false;
		...
		public override string GetChat() {
			...
			if (partyGirl >= 0) {
				chat.Add(Language.GetTextValue("汪汪汪", Main.npc[partyGirl].GivenName));
			}

			chat.Add(Language.GetTextValue("汪"));

			string hivePackDialogue = Language.GetTextValue("汪汪");
